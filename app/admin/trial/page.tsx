...
export default function TrialPage() {
  ...
  const handleLookup = async () => {
    ...
    try {
      ...
      if (!res.ok) { setLookupError(data.error || "Erro ao buscar lead") }
      else { setLeadInfo(data) }
    } catch { setLookupError("Erro de conexao") }
  ...
  const handleActivate = async () => {
    ...
    try {
      ...
      const data = await res.json()
      if (!res.ok) { setActivationError(data.error || "Erro ao ativar trial") }
      else { setActivationResult(data) }
    } catch { setActivationError("Erro de conexao") }
    finally { setLoading(false) }
  }

  const quizLabels: Record<string, string> = {
    goal: "Objetivo", weight: "Peso atual (kg)", targetWeight: "Peso alvo (kg)",
    height: "Altura (cm)", age: "Idade", gender: "Genero",
    activityLevel: "Nivel de atividade", workoutTime: "Tempo de treino",
    workoutDays: "Dias de treino/semana", workoutLocation: "Local de treino",
    fitnessLevel: "Nivel fitness", dietaryRestrictions: "Restricoes alimentares",
  }
        ...
        <div className="max-w-2xl mx-auto space-y-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-slate-400 text-sm">Ative 15 dias de acesso para leads que chegaram ao checkout</p>
            </div>
          </div>
          ...
          {activationResult && (
            <Card className="bg-slate-800 border-slate-700 p-6 space-y-4">
              <h2 className="text-white font-semibold text-lg text-green-400">Trial Ativado com Sucesso!</h2>
              ...
              {activationResult.isNewUser && activationResult.tempPassword && (
                <div className="bg-lime-500/10 border border-lime-500/30 rounded-lg p-4">
                  <p className="text-lime-400 text-sm font-semibold mb-2">Conta criada - anote a senha temporaria:</p>
                  ...
                  <p className="text-slate-400 text-xs mt-2">Entre na conta para verificar antes de enviar o email ao usuario.</p>
                </div>
              )}
              {!activationResult.isNewUser && (
                <div className="bg-slate-700/50 rounded-lg p-3">
                  <p className="text-slate-300 text-sm">Usuario ja existia. A senha nao foi alterada.</p>
                </div>
              )}
              {activationResult.plansError && (
                <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-lg p-3">
                  <p className="text-yellow-400 text-sm">Planos nao gerados automaticamente - gere manualmente na pagina do usuario.</p>
                </div>
              )}
              <div className="flex gap-3 pt-2">
                ...
                <Button onClick={() => { setEmail(""); setLeadInfo(null); setActivationResult(null); setActivationError(null) }} className="flex-1 bg-lime-600 hover:bg-lime-500 text-white">Ativar Outro</Button>
              </div>
            </Card>
          )}
        </div>

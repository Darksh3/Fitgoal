...
export async function POST(request: Request) {
  ...
  try {
      ...
      if (leadsSnap.empty) {
              return NextResponse.json({ error: "Nenhum lead encontrado com esse email" }, { status: 404 })
      }
      ...
      // ── 3. Criar ou atualizar usuário no Firebase Auth ─────────────────────────
      let userId = leadId
      ...
      try {
          ...
          firebaseUser = await auth.getUserByEmail(cleanEmail)
              ...
              // Se já existe, não altera a senha (usuário pode já ter configurado a própria)
      } catch {
      ...
      const isNewUser = !firebaseUser?.metadata?.lastSignInTime

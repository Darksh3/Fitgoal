...
export async function POST(request: Request) {
  ...
  try {
          ...
          // FIX: usar o docId correto — se já existe documento no Firestore com outro UID,
          // atualizar ESSE documento para evitar duplicidade e garantir que o usuário
          // ao fazer login carregue os dados corretos.
          const firestoreDocId = existingUserId || userId

          // Se for apenas extensão de dias e o usuário já existe no Firestore, manter o plano atual.
          ...
          if (appUrl) {
                      try {
                        ...
                        if (plansRes.ok) {
                                    ...
                                    plansError = err?.name === "AbortError"
                                      ? "Timeout ao gerar planos"
                                                    : (err?.message || "Erro ao gerar planos")
                      }
          } else {
                      plansError = "APP_URL não configurada — planos não foram gerados automaticamente"
          }

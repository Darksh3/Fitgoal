...
export async function POST(request: Request) {
  ...
  if (userId) {
    console.log("[v0] WEBHOOK_PROCESSING - Iniciando processamento com timeout")

    const TIMEOUT_MS = 8000
    await Promise.race([
      processPaymentBackground(payment, userId),
      new Promise((_, reject) => setTimeout(() => reject(new Error("BG timeout")), TIMEOUT_MS)),
    ]).catch((err) => {
      console.error("[v0] WEBHOOK_BG_TIMEOUT_OR_ERROR - Erro/timeout:", err)
    })
  }
}

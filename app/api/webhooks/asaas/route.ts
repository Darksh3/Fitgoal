  ...
  try {
    ...
    switch (event) {
      ...
      case "PAYMENT_CONFIRMED":
        console.log("[v0] WEBHOOK_PAYMENT_EVENT - Pagamento confirmado com event:", event)
        
        // PRIMEIRO: Atualizar o Firestore com o novo status
        try {
          console.log("[v0] WEBHOOK_UPDATING_PAYMENT - Atualizando status do pagamento no Firestore")
          console.log("[v0] WEBHOOK_PAYMENT_ID - Payment ID para atualizar:", payment?.id)
        ...
        if (!userId) {
          console.warn("[v0] WEBHOOK_NO_USER_ID - Pagamento atualizado mas sem userId para processar")
          return NextResponse.json({ received: true, status: "updated" }, { status: 200 })
        }
        ...
        try {
          ...
          console.error("[v0] WEBHOOK_ERROR - Erro ao processar:", error)
        }

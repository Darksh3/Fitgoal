          ...
          <div className="grid grid-cols-2 gap-8 mt-8 pt-8 border-t border-gray-800">
            ...
            <div className="bg-gray-900 bg-opacity-50 rounded-lg p-6 border border-gray-800">
              <div className="space-y-4">
                ...
                <div>
                  {/* Nível de Treino (card da esquerda) */}
                  <p className="text-gray-400 text-sm mb-1">Nível de Treino</p>

                  {/*
  ✅ Sem mexer no case 15:
  - Se "experience" vier vazio, cai automaticamente em "strengthTraining" (case 15)
  - Se ainda não existir, assume "beginner"
*/}
                  {(() => {
                    const rawExperience =
                    ...
                    const normalizeExperienceForBar = (v: string) => {
                      const val = (v || "").toLowerCase();
                      if (val === "beginner") return "iniciante";
                      if (val === "intermediate") return "intermediário";
                      if (val === "advanced") return "avançado";
                      return v; // se já vier em PT, mantém
                    };

                    return (
                      <>
                        <p className="text-white font-bold mb-2">{experienceLabel(rawExperience)}</p>
                        <div className="w-full bg-gray-800 rounded-full h-2 overflow-hidden">
                          <motion.div
                            initial={{ width: 0 }}
                            whileInView={{
                              width: `${getTrainingLevelPercentage(
                                normalizeExperienceForBar(rawExperience)
                              )}%`,
                            }}
                  ...
                  {/* Target stats - Nível de Treino (card da direita) */}
                  <div>
                    <p className="text-gray-400 text-sm mb-1">Nível de Treino</p>
                    ...
                    {(() => {
                      ...
                      const nextTrainingLabel = (v: string) => {
                        const val = (v || "").toLowerCase();
                        // ✅ Mantém a sua lógica (iniciante -> intermediário, senão -> avançado),
                        // mas agora aceitando também os valores do case 15 (beginner/intermediate/advanced)
                        if (val === "iniciante" || val === "beginner") return "Intermediário";
                        return "Avançado";
                      };

                      return (
                        <>
                          <p className="text-white font-bold mb-2">{nextTrainingLabel(rawExperience)}</p>
                          <div className="w-full bg-gray-800 rounded-full h-2 overflow-hidden">
                            <motion.div
                              initial={{ width: 0 }}
